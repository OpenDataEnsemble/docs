...
function validateConfigPaths(docFiles: string[], result: ValidationResult): void {
  for (const docPath of extractDocPathsFromConfig()) {
    ...
    if (
      !docFileExists(relativePath, docFiles) &&
      !docFileExists(`${relativePath}/index`, docFiles)
    ) {
      result.warnings.push(
        `Path "${docPath}" in docusaurus.config.ts may not exist (verify manually)`
      );
    }
  }
}
      ...
      if (entry.isDirectory()) {
        ...
        while ((match = linkRegex.exec(content)) !== null) {
          ...
          if (!docFileExists(docPath, docFiles) && !docFileExists(`${docPath}/index`, docFiles)) {
            result.warnings.push(
              `${relativePath}: Link "${linkText}" points to "${linkPath}" which may not exist`
            );
          }
        }
      }
